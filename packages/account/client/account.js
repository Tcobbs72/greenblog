...
Template.account.helpers({
    ...
    posts: function(){
        if(!Meteor.user() || Meteor.user().username != Session.get("selectedAccount")) return Posts.find({author: Session.get("selectedAccount"), anonymous: false}, {sort: {date: -1}}).fetch();
        else return Posts.find({author: Session.get("selectedAccount")}, {sort: {date: -1}}).fetch();
    },
